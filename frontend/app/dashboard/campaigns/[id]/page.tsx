...
export default function CampaignDetailPage() {
  ...
  // Real backend data only. (The mock/empty preview modes were removed so a
  // client never sees fabricated campaign numbers.)
  const [dataMode] = useState<DataMode>("api");
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto space-y-8 pb-10">
      ...
      {/* Header — only once real data has loaded, so no mock flashes */}
      {apiData && (
        <CampaignHeader
          data={data.header}
          busy={actionLoading}
      ...
      {!apiData ? (
        loading ? (
          ...
          <div className="w-full bg-white rounded-2xl border border-slate-200/70 p-10 text-center">
            <p className="text-sm font-semibold text-slate-700 mb-1">Couldn&apos;t load this campaign</p>
            <p className="text-xs text-slate-400 mb-5">
              {error || "The backend didn't respond. Apply the DB migration and restart the API."}
            </p>
            <button
              onClick={refetch}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl bg-slate-900 text-white hover:bg-slate-800 transition-all"
            >
              Retry
            </button>
          </div>
